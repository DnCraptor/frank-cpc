...
extern t_CPC CPC;
extern t_FDC FDC;
/* driveA/driveB are macros defined in disk.h → driveA_p/driveB_p */

#define MAX_TRACK_SIZE 6144
...
t_sector *find_sector(byte *requested_CHRN)
{
   ...
   sector = 0; // return value indicates 'sector not found' by default
...
inline void cmd_write()
{
   ...
   if (sector) { // sector found
      ...
      FDC.buffer_ptr = t_sector_getDataForWrite(sector); // pointer to sector data
...
loop:
   ...
   if (sector) { // sector found
      ...
      if ((FDC.flags & SKIP_flag) && (FDC.result[RES_ST2] &= 0x40)) { // skip sector?
         ...
         FDC.buffer_count = sector_size; // init number of bytes to transfer
         FDC.buffer_ptr = t_sector_getDataForRead(sector); // pointer to sector data
         FDC.buffer_endptr = active_track->data + active_track->size; // pointer beyond end of track data
         FDC.timeout = INITIAL_TIMEOUT;
         read_status_delay = 1;
      }
               ...
               if ((FDC.flags & SCAN_flag)) { // processing any of the scan commands?
                  ...
                  if (((sector_size + 62 + FDC.command[CMD_R]) * FDC.command[CMD_H]) > CPC.max_tracksize) { // track size exceeds maximum?
                     ...
                     for (sector = 0; sector < FDC.command[CMD_H]; sector++) {
                        ...
                        t_sector_setData(&active_track->sector[sector], pbDataPtr); // store pointer to sector data
                        t_sector_setSizes(&active_track->sector[sector], sector_size, sector_size);
