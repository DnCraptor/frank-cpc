...
// Host allocates SCREEN[2][graphics_buffer_width * graphics_buffer_height];
// the driver only uses the width stride at runtime, so the exact C array
// dimensions are opaque here.
...
static void __scratch_y("hdmi_driver") dma_handler_HDMI() {
    ...
    /* Clear the interrupt status in the correct register.
     * VIDEO_DMA_IRQ selects which IRQ the DMA channel fires on; the
     * corresponding ints0/ints1 register must be written to re-arm the
     * edge-triggered signal so the ISR fires on every completion. */
    if (VIDEO_DMA_IRQ == DMA_IRQ_0)
        dma_hw->ints0 = 1u << dma_chan_ctrl;
    else
        dma_hw->ints1 = 1u << dma_chan_ctrl;
    ...
    // Content scanlines: the 400-byte DMA buffer layout is:
    //   [0..47]   = 48 sync/control bytes (hsync front porch + pulse)
    //   [48..71]  = 24 sync/control bytes
    //   [72..391] = 320 pixel bytes  (EXACTLY fits 320-wide content, no margins)
    //   [392..399]= 8 trailing sync bytes
    // For narrower content (e.g. 256px) we centre with equal left/right margins.
    // For 320px content there is no room for margins — skip them to avoid
    // overflowing the buffer and corrupting adjacent memory / sync structure.
    if (line < (VMARGIN_SCANLINES + CONTENT_SCANLINES + VMARGIN_SCANLINES) ) {
        // Active video region
        const int content_w = graphics_buffer_width;
        const int left_margin = (320 - content_w) / 2;

        uint8_t* output_buffer = activ_buf + 72; // Align sync

        // Left margin (black) — zero bytes when content is 320px wide
        if (left_margin > 0) {
            hdmi_memset_fast(output_buffer, 0, (uint32_t)left_margin);
            output_buffer += left_margin;
        }
        ...
        if (snes_scanline >= 0 && snes_scanline < CONTENT_SCANLINES) {
            ...
            const uint8_t* input = &SCREEN[!current_buffer][(snes_scanline / 2) * content_w];

            // Copy pixels using optimized assembly routine
            hdmi_copy_scanline_asm(output_buffer, input, (uint32_t)content_w, hdmi_color_substitute);
        } else {
            // Top/bottom margin: black
            hdmi_memset_fast(output_buffer, 0, (uint32_t)content_w);
        }
        output_buffer += content_w;

        // Right margin (black) — zero bytes when content is 320px wide
        if (left_margin > 0) {
            hdmi_memset_fast(output_buffer, 0, (uint32_t)(320 - content_w - left_margin));
        }
